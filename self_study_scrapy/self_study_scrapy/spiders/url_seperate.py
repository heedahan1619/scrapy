...
class Stella2Spider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        print(f"크롤링 시작 날짜: {self.start_date}")  
        print(f"크롤링 종료 날짜: {self.end_date}")  
        print("\n")
    ...
    def parse_media_category(self, response):   
        ...
        for media_category, media_category_sub_dict in self.media_dict.items():
            ...
            for media_category_sub_code, media_category_sub in media_category_sub_dict.items():
                ...
                # print(f"\n소분류: {media_category_sub}")
                
                # 페이지 지정
                for page_num in range(1, self.page_request_range+1):
                    
                    """url 구분"""
                    # 페이지 없는 카테고리- 시세, 장외/IPO
                    if (media_category_sub == "시세 ") or (media_category_sub == "장외/IPO "):
                        url = self.async_category_list_url.format(media_category_sub_code)
    ...
    def parse_category_news_list(self, response):
        ...
        # print(f"\n카테고리별 뉴스 목록 페이지 url: {response.url}")
        ...
        # 페이지 없는 카테고리
        if (media_category_sub == "시세 ") or (media_category_sub == "장외/IPO "):
            response.url = "https://www.etoday.co.kr/"+ media_category_sub
            is_next_news_list = False    
            # print(f"페이지가 없는 url: {response.url}")
            
        # 페이지 있는 카테고리
        else:
            if response.url == "https://www.etoday.co.kr/"+ media_category_sub:
                page_num = 1
            else:
                page_num = (response.url).split('=') [-1]
            is_next_news_list = True
            # print(f"페이지가 있는 url: {response.url}")
    
        # print(f"페이지 처리 뉴스 목록 url: {response.url}") 
        ...
        for news_list in response.xpath("//ul[@id='list_W']/li"):
            ...
            if self.start_date <= date <= self.end_date:
                print(f"\n 기간 내 뉴스 리스트 url: {response.url}")
                print(f"기간 내 뉴스 날짜: {date}")
                ...
                # else:
                #     is_next_news_list = False
                # print(f"다음 페이지를 확인하나?: {is_next_news_list}")
