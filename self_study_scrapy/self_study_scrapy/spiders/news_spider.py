...
class SmedailySpider(scrapy.Spider):
    ...
    """url"""
    # 메인 홈페이지 url
    main_url = "https://www.smedaily.co.kr"
    
    # 카테고리 뉴스 목록 url
    category_news_list_url = "https://www.smedaily.co.kr{}&page={}"
    ...
    def parse_media_category(self, response):   
        ...
        """카테고리 대분류"""
        for category_info in response.xpath("//ul[@id='user-menu']/li/a"):
            # print(f"{self.media} 대분류 카테고리 정보: {category_info}")
            
            # 대분류 카테고리 코드
            media_category_code = category_info.xpath("@href").get()
            # print(f"{self.media} 대분류 카테고리 코드: {media_category_code}")  
            
            # 대분류 카테고리 이름
            media_category = category_info.xpath("text()").get()
            input_media_category += (media_category + ", ")
            # print(f"{self.media} 대분류 카테고리: {media_category}")
            # print("--------------------------------")
        ...
        print(f"{self.media} 대분류 카테고리 dict: {self.media_category_dict}")
        
    
        """카테고리 입력"""
        # 카테고리 입력 시 카테고리 정보 제공을 위한 문자열
        input_media_category = input_media_category[:-2]     
        # print(f"선택할 카테고리 문자열: {input_media_category}")
        
        # 카테고리 선택
        select_media_category = input(f"\n[{input_media_category}] 중에서 카테고리를 선택하세요: \t").split(", ")
        print(f"선택한 카테고리 전체:  {select_media_category}")
        
        # 선택한 카테고리가 언론사 카테고리에 존재하는지 확인
        ...
        """스크래피 요청"""    
        # 카테고리 추출
        for media_category_code, media_category in self.media_category_dict.items():
            # print(f"{self.media} 대분류 카테고리 코드: {media_category_code}")
            # print(f"{self.media} 대분류 카테고리: {media_category}")
            # print("--------------------------------")
            ...
            for page_num in range(1, self.page_request_range+1):
                ...
                # print(self.category_news_list_url.format(media_category_code, page_num))
                        
                # 스크래피 요청
                yield scrapy.Request(
                    
                    # 카테고리 뉴스 목록 url
                    url = self.category_news_list_url.format(media_category_code, page_num),
    ...
    def parse_category_news_list(self, response):
        ...
        # print(f"{self.media} 카테고리 뉴스 목록 url: {response.url}")
        
        media_category_code = response.meta["media_category_code"]
        media_category = response.meta["media_category"]
        media_category_sub = response.meta["media_category_sub"]
        
        # print(f"\n{self.media} 카테고리 대분류: {media_category}")
        # print(f"{self.media} 카테고리 소분류: {media_category_sub}")
        # print(f"{self.media} 카테고리 대분류 코드: {media_category_code}")
        
        
        # 페이지 처리
        if response.url == self.main_url + media_category_code:
            page_num = 1
        else:
            page_num = (response.url).split('=')[-1]
        # print(f"페이지 번호: {page_num}")
        # print(f"\카테고리 뉴스 목록 url: {response.url}")
        # print("--------------------------------")
        ...
        # 카테고리별 뉴스 목록 확인
        for news_list in response.xpath("//section[@id='section-list']//div[@class='view-cont']"):
            # print(f"\n뉴스 목록 정보: {news_list}")
        
            # 날짜 추출
            date = news_list.xpath("span[@class='byline']/em[3]/text()").get()
            # print(f"{self.media} 카테고리별 뉴스 목록 날짜: {date}")
            
            date = datetime.strptime(date, "%Y.%m.%d %H:%M")
            # print(f"{self.media} 카테고리별 뉴스 목록 datetime 날짜: {date}")
        
            # 뉴스 기사 url
            news_post_url = self.main_url + news_list.xpath("h4[@class='titles']/a/@href").get()
            # print(f"{self.media} 뉴스 기사 url: {news_post_url}")
            ...
            if is_next_news_list == True:
                ...
                yield scrapy.Request(
                    ...
                    # 카테고리 뉴스 목록 url
                    url=self.category_news_list_url.format(media_category_code, str(next_page_num)),
                    ...
                    meta={
                            "media_code": media_category_code,
                            "media_category": media_category,
                            "media_category_sub":media_category_sub
                    } 
                )   
    ...
    def parse_category_news_post(self, response):
        ...
        # 기사 제목
        item["title"] = response.xpath("//header[@class='article-view-header']/h3[@class='heading']/text()").get().strip()
        print(f" 기사 제목: {item["title"]}")
        
        # 기사 내용
        item["content"] = " ".join(response.xpath("//div[@class='article-body']/article/p/text()").getall()).replace("\n", "").strip()    
